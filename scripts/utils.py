...
# Use mlx-lm's model loading infrastructure for multi-architecture support
from mlx_lm.utils import load_model, load_config


def fetch_from_hub(hf_path: str):
    """
    Fetch model weights, config, and tokenizer from Hugging Face Hub.
    """
...
def load(path_or_hf_repo: str, tokenizer_config={}):
    ...
    This function uses mlx-lm's model loading infrastructure which supports
    multiple model architectures including:
    - Llama (1, 2, 3, 4)
    ...
    # Load config to get model type info
    config = load_config(model_path)
    
    # Use mlx-lm's model loading which handles all architectures
    model, loaded_config = load_model(model_path, lazy=False)
...
def generate(
    ...
    Works with all mlx-lm supported model architectures (Llama, Mistral, Gemma, etc.)
    ...
    # Initialize cache - use model's make_cache if available, otherwise create KVCache for each layer
